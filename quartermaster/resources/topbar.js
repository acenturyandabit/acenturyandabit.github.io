// V0.2 top bar (file bar? idk what to call it) manager.
...
function _topbarManager(userSettings) {
    ...
    this.checkTopbars = function (root) {
        if (!root) root = document.body;
        let els = root.querySelectorAll("ul.topbar");
        for (let i = 0; i < els.length; i++) {
            let root = els[i];
            let lis = root.querySelectorAll('li');
            for (let j = 0; j < lis.length; j++) {
                //there are any elements other than a's and ul's
                let stylingOK = true;;
                for (let k = 0; k < lis[j].childNodes.length; k++) {
                    let toInsertNode = lis[j].childNodes[k];
                    if (toInsertNode.nodeName.toLowerCase() != "ul" && toInsertNode.nodeName.toLowerCase() != "a") {
                        stylingOK = false;
                    }
                }
                //wrap every junk into an a.
                if (!stylingOK) {
